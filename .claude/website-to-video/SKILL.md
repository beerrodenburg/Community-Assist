...
description: Turn a video into a premium scroll-driven animated website with GSAP, canvas frame rendering, and layered animation choreography. Built for this project — uses the local serve.mjs / screenshot.mjs and respects brand_assets/.
---

# Video → Premium Scroll-Driven Website
...
**Always check `brand_assets/` first.** If logos, color palettes, or style guides exist there, use them — do not invent brand colors or use placeholder logos when real assets are available.
...
**FFmpeg and FFprobe are installed via Homebrew at `/opt/homebrew/bin/` and on PATH.** Verify with `which ffmpeg ffprobe` if needed. Do NOT reinstall.
...
This skill produces a multi-file build (HTML/CSS/JS + frames). That overrides the project default of "single index.html with all styles inline" — frame-by-frame canvas rendering needs a `frames/` directory served over HTTP, so a split layout is required.
...
Invoke the **frontend-design skill** for creative, distinctive styling. Key technical patterns:
...
### Step 7: Test (use the project's local server + screenshot tools)

The project ships with `serve.mjs` and `screenshot.mjs` in the root. Use those — do NOT use `npx serve` or `python -m http.server`, and do NOT screenshot a `file:///` URL (frames won't load over `file://` anyway).

1. **Start the server** in the background (skip if already running):
   ```bash
   node serve.mjs
   ```
   Serves the project root at `http://localhost:3000`.

2. **Scroll-test in the browser** at `http://localhost:3000` — verify each section has a DIFFERENT animation type. Confirm: smooth scroll, frame playback, staggered reveals, marquee slides, counters count up, dark overlay fades, CTA persists at end.

3. **Screenshot** for visual review (auto-saves into `temporary screenshots/`):
   ```bash
   node screenshot.mjs http://localhost:3000
   node screenshot.mjs http://localhost:3000 mid-scroll   # optional label
   ```
   Then read the PNG with the Read tool and compare against any reference. Do at least 2 comparison rounds, fix mismatches, re-screenshot.
...
- **Default Tailwind palette** — per project rules, never use indigo-500/blue-600/etc. as primary. Derive from `brand_assets/`.
...
- **Frames not loading**: Must serve via HTTP — use `node serve.mjs`, not `file://`
...
- **Port 3000 already in use**: A previous `serve.mjs` is still running — reuse it instead of starting a second instance
